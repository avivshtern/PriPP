...
class avl_tree
{
	...
	Key         key_[Size];                 // the key we use to sort the terr
	...
	{
		avl_tree* instance_;    // array instance
		size_type   idx_;         // actual node

		friend avl_tree;         // avl_array may access index pointer

	public:
		// ctor
		tag_avl_array_iterator(avl_tree* instance = nullptr, size_type idx = 0U)
			: instance_(instance)
			, idx_(idx)
		{ }
